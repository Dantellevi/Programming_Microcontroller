...
{
  uint16_t status;
	GPIOB->ODR &= ~GPIO_ODR_ODR0;//низкий уровень
  DelayMicro(520);//задержка как минимум на 480 микросекунд
  GPIOB->ODR |= GPIO_ODR_ODR0;//высокий уровень
  DelayMicro(70);//задержка как минимум на 60 микросекунд
  status = GPIOB->IDR & GPIO_IDR_IDR0;//проверяем уровень
  DelayMicro(425);//задержка как минимум на 480 микросекунд
...
uint8_t ds18b20_ReadBit(void)
{
  ...
  DelayMicro(3);
	GPIOB->ODR |= GPIO_ODR_ODR0;//высокий уровень
	DelayMicro(13);
	bit = (GPIOB->IDR & GPIO_IDR_IDR0 ? 1 : 0);//проверяем уровень	
	DelayMicro(45);
  return bit;
}
//-----------------------------------------------
