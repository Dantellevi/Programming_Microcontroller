...
NRF_RESULT nrf_set_rx_address_p1(nrf24l01* dev, const uint8_t* address) 
	{
    uint8_t rx[5];
    if (nrf_send_command(dev, (NRF_CMD_W_REGISTER | (NRF_COMMAND)NRF_RX_ADDR_P1), address, rx,
                         5) != NRF_OK) {
        return NRF_ERROR;
    }
    dev->config.rx_address = address;
...
NRF_RESULT nrf_set_tx_address(nrf24l01* dev, const uint8_t* address) 
	{
    uint8_t rx[5];
    if (nrf_send_command(dev, (NRF_CMD_W_REGISTER | (NRF_COMMAND)NRF_TX_ADDR), address, rx,
                         5) != NRF_OK) {
        return NRF_ERROR;
    }
